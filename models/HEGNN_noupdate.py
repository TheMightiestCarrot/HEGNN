...
class HEGNN_Layer(nn.Module):
    ...
    def forward(self, node_feat, node_pos, node_sh, node_vel, edge_index, edge_attr, diff_pos, dist):
        msg, scaled_diff_pos, diff_sh = self.Msg(edge_index, edge_attr, node_feat, node_sh, diff_pos, dist)
        msg_agg, pos_agg, sh_agg = self.Agg(edge_index, node_feat.size(0), msg, scaled_diff_pos, diff_sh)
    ...
    def Msg(self, edge_index, edge_attr, node_feat, node_sh, diff_pos, dist):
        ...
        return msg, scaled_diff_pos, diff_sh
    ...
    def Upd(self, node_feat, node_pos, node_sh, node_vel, msg_agg, pos_agg, sh_agg):
        # Update steerable reps and invariant features; keep positional update behavior.
...
class HEGNN(nn.Module):
    ...
    def forward(self, node_feat, node_pos, node_vel, edge_index, edge_attr):
        ...
        # Precompute geometric quantities once and reuse in all layers.
        row, col = edge_index
        diff_pos = node_pos[row] - node_pos[col]
        dist = torch.norm(diff_pos, p=2, dim=-1).unsqueeze(-1) ** 2

        for layer in self.layers:
            node_feat, node_pos, node_sh = layer(
                node_feat,
                node_pos,
                node_sh,
                node_vel,
